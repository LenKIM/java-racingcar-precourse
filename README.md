...
1. - [ ] 각 자동차는 전진 또는 멈춤을 할 수 있다.
     - [ ] 각 자동차는 5자이하의 이름을 가진다.
     - [ ] 처음 위치는 시작점이여야 한다.
     - [ ] 자동차는 전진 또는 멈춤 할 수 있는 조건을 가진 엑셀레이터(accelerator)를 가진다.
     - [ ] 엑셀레이터(accelerator)는 랜덤 숫자 결과에 따라 0-9 사이의 값 중, 4 이상일 경우 전진 또는 이하의 값일 경우, 멈춤한다.
